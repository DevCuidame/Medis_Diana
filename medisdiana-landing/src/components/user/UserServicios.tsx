...
export const UserServicios: React.FC<Props> = () => {
  ...
  const handleEnroll = async (
    ...
    opts?: { paymentMethod?: 'cash' | 'wompi'; expectedAmount?: number; discountPct?: number; discountCode?: string }
  ) => {
    ...
    <main style={{ flex: 1, overflowY: 'auto', background: C.bg }}>
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '32px 28px' }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 24, flexWrap: 'wrap' }}>
          <input value={discountCode} onChange={e => setDiscountCode(e.target.value.toUpperCase())} placeholder="Código de descuento (opcional)"
            style={{ padding: '9px 14px', borderRadius: 10, border: '1.5px solid #DDD6FE', background: C.white, fontSize: 12, fontFamily: 'monospace', color: C.text, outline: 'none', maxWidth: 240 }}
          />
          {discountCode && <span style={{ fontSize: 11, color: C.textMuted }}>Se aplicará al confirmar tu inscripción</span>}
        </div>
                  ...
                  <div style={{ padding: '0 20px 18px' }}>
                    {(() => {
                      ...
                      if (hasFreeForThis) {
                        ...
                        return (
                          <button onClick={() => handleEnroll(g.ids, g.key, { discountCode: discountCode.trim() || undefined })} disabled={enrolling === g.key}
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                  ...
                  <button
                    onClick={() => {
                      ...
                      setLimitModal(null)
                      handleEnroll(m.ids, m.key, { paymentMethod: payMethod, expectedAmount: paidTotal, discountPct: m.discountPct ?? undefined, discountCode: discountCode.trim() || undefined })
                    }}
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                  ...
                  <button
                    onClick={() => {
                      ...
                      handleEnroll(c.ids, c.key, { paymentMethod: payMethod, expectedAmount: amt, discountPct: c.discountPct, discountCode: discountCode.trim() || undefined })
                    }}
                    style={{ flex: 2, padding: '12px', borderRadius: 10, border: 'none', background: 'linear-gradient(135deg, #B45309, #D97706)', color: C.white, fontSize: 13, fontWeight: 700, cursor: 'pointer', fontFamily: FONT_INTER }}>
                    {payMethod === 'wompi' ? '💳 Pagar con Wompi' : '✓ Confirmar inscripción'}
                  </button>
